...
if selected_sheet not in st.session_state.sheet_cache:
    ...
    if not _cached:
        with st.spinner(f"Reading '{selected_sheet}'…"):
            ...
            # Reuse the already-parsed result stored during upload
                
                data            = []   # entities are served from _pdf_intelligence, not data rows
            ...
            # ── Excel / CSV branch ────────────────────────────────────────────
                ...
                try:
                    ...
                    from modules.audit import _log_error
                    _log_error("EXCEL_PARSE", uploaded.name, _e)
                    st.error(f"⚠ Could not parse this Excel file: {_e}")
                    st.stop()
...
if data and file_ext != ".pdf" and sheet_type != "CHECK_REGISTER":
    ...
    if _needs_llm_map:
        try:
            ...
            _to_remember += [
                {"source_column": c, "resolved_field": f, "method": "llm", "confidence": 75.0}
